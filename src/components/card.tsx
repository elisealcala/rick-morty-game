...
export const Card: FC<CardProps> = ({ image, name, status, species, blocked, flipped, onClick }) => {
  const className = flipped ? 'bg-[#A2F2F9] cursor-pointer h-[260px] flex items-center' : 'bg-white'

  return (
    <div onClick={!blocked && flipped ? onClick : () => null} className={`w-full h-full p-4 rounded ${className}`}>
      {flipped ? (
